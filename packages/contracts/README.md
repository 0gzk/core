...
# install forge-std (one-time, vendored under lib/)
cd packages/contracts
forge install foundry-rs/forge-std --no-commit
```

`forge-std` is vendored as a git submodule under `packages/contracts/lib/forge-std/`. If you cloned without submodules, run `git submodule update --init --recursive` from the repo root.
